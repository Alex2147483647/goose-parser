 ...
 * @property {Array.<string>} cookieDomains
 */
...
function PhantomEnvironment(options) {
    ...
    this._cookies = this._options.cookies || false;
    this._cookieDomains = this._options.cookieDomains || null;
    this._cookieJar = {};
...
PhantomEnvironment.prototype = _.create(Environment.prototype, /**@lends PhantomEnvironment*/{
    ...
    _handlePhantomEvents() {
        ...
        page.set('onNavigationRequested', (url, type, willNavigate, mainFrame) => {
            debug('Navigation to %s %s %s %s', url, type, willNavigate, mainFrame);
            let i = 0;
        ...
        page.onResourceRequested(/* @covignore */ function(requestData, request, allowedUrls, blockedUrls, cookieJar) {
            var getRequestCookies = function(url) {
                return Object.keys(cookieJar).reduce(function(reduced, path) {
                    const cookieDomain = cookieJar[path];
                    if (url.match(path)) {
                        const pairs = Object.keys(cookieDomain).map(function(key) {
                            const cookie = cookieDomain[key];
                            // TODO: check expires if exists
                            return cookie.name + '=' + cookie.value;
                        });
                        Array.prototype.push.apply(reduced, pairs);
                    }
                    return reduced;
                }, []);
            };

            var cookies = getRequestCookies(requestData.url);
            console.log(requestData.url, cookies.join('; '));
            if (cookies.length > 0) {
                request.setHeader("Cookie", cookies.join('; '));
            }
        ...
        }, this._options.resources.allowed, this._options.resources.denied, (this._cookies === true ? this._cookieJar : {}));

        page.set('onResourceReceived', (resource) => {
            // debug('Resource recieved %o', resource);
            if (this._cookies === true && this._cookieDomains !== null) {
                const domain = this._getHostName(resource.url);
                const condition = this._cookieDomains === 'all' ||
                    (
                        Array.isArray(this._cookieDomains) &&
                        _.some(this._cookieDomains, (cookieDomain) => domain.match(cookieDomain))
                    );
                if (condition) {
                    let cookies = resource.headers || [];
                    cookies = _.filter(cookies, {'name': 'Set-Cookie'});
                    if (cookies.length > 0) {
                        _.each(cookies, (cookie) => this._addCookies(cookie.value, domain));
                    }
                }
            }
    ...
    collectJsCookies() {
        ...
        debug('.collectJsCookies()');
        return this.evaluateJs(/* @covignore */ function() {
            return document.cookie;
        }).then(cookies => {
                cookies = cookies.split(';');

                let domain = this._getHostName(this._options.url).match(/(([^\.]+)\.([^\.]+))$/);
                domain = domain[0];

                _.each(cookies, (value) => {
                    const cookie = this._createCookieFromString(value);
                    this._addCookieToJar(cookie, domain, true);
                });
            },
            (e) => {
                // do not present this error in global stack
                debug('Error during attempt to get js cookies: %s %s', e.message, e.stack);
            }
        );
    },

    _addCookies(cookies, domain) {
        cookies = cookies.split('\n');
        _.each(cookies, (cookieString) => {
            const cookie = this._createCookieFromString(cookieString);
            this._addCookieToJar(cookie, domain);
        });
    },

    _addCookieToJar(cookie, domain, setValueOnly) {
        setValueOnly = setValueOnly || false;
        if (cookie.name === '') {
            return;
        }
        const path = (cookie.options.domain ? _.trimStart(cookie.options.domain, '.') : domain) +
            (cookie.options.path && cookie.options.path !== '/' ? cookie.options.path : '');
        if (!this._cookieJar[path]) {
            this._cookieJar[path] = {};
        }
        if (setValueOnly && this._cookieJar[path][cookie.name] !== undefined) {
            this._cookieJar[path][cookie.name].value = cookie.value;
        }
        else {
            this._cookieJar[path][cookie.name] = cookie;
        }
    },

    _createCookieFromString(cookieString) {
        let cookie = cookieString.trim().split(';');
        const cookieName = cookie[0].split('=')[0];
        cookie = cookie.reduce((reduced, cookieOption) => {
            cookieOption = cookieOption.trim().split('=');
            switch (cookieOption.length) {
                case 1:
                    // add default value, if specified only key for cookie option
                    cookieOption[1] = true;
                    break;
                case 2:
                    break;
            ...
            const cookieOptionName = cookieOption[0];
            const cookieOptionValue = cookieOption[1];

            if (cookieName === cookieOption[0]) {
                reduced.name = cookieOptionName;
                reduced.value = cookieOptionValue;
            }
            else {
                reduced.options[cookieOptionName.toLowerCase()] = cookieOptionValue;
            }
            return reduced;
        }, {
            options: {}
