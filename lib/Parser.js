...
Parser.prototype = {
    ...
    parse: function(options) {
        ...
        return this._env.prepare()
            ...
            .then(this._env.collectJsCookies, e => {
                return this._env.collectJsCookies()
                    .then(() => {
                        throw e
                    });
            }, this._env)
     ...
     * @returns {Promise}
     */
    _processRule: function(rule, offset) {
    ...
    _parseScope: function(rule, offset, actionsResults) {
        ...
        const format = results => {
            if (_.includes([this.TYPES.SIMPLE, this.TYPES.GET, this.TYPES.VALUE, this.TYPES.COLLECTION], ruleType)) {
        ...
        const setResultsToStore = results => {
            if (rule.set) {
    ...
    _parseSimpleRule: function(rule) {
        ...
        debug('._parseSimpleRule() has called for selector %s %o', selector, filter);
